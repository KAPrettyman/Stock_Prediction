...
def convert_input_pca_regression(request_body, request_content_type):
    ...
    if option == 2:
        ...
        X = FeatureEngineer(windows=[10,15,20,25,30]).transform(dataset[[target]])
    
        techIndicator_1 = 'MOM_15'
        MOM_15 = json.loads(request_body)[techIndicator_1]
        techIndicator_2 = 'ROC_10'
        ROC_10 = json.loads(request_body)[techIndicator_2]

        # Calculate the distance
        distances = np.sqrt(
            (X[techIndicator_1] - MOM_15)**2 + 
        ...
        closest_row[techIndicator_1] = MOM_15
